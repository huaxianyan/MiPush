...
本分支为 `com.tencent.mobileqq` 内置了符合 Android 通知规范的单色小图标。MiPush 代理发布的 QQ 通知会自动使用该兜底图标，不需要开启“自定义通知图标”；开启该选项后，用户配置的自定义图标仍具有更高优先级。Android 16 的通知界面默认会用 QQ 彩色启动器图标覆盖联系人头像右下角的小图标，本分支会让该位置继续使用通知单色小图标，与 Android 13 的显示行为一致；联系人或群头像本身不会被覆盖。
